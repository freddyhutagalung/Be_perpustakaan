...
const { selectAll, selectSearchpenerbit, select, insert, update, deleteData, countData, findId } = require("../model/penerbit");
const commonHelper = require("../helper/common");
const cloudinary = require("../middlewares/cloudinary");

const Joi = require("joi");
// const user = require('../controller/users')
const penerbitController = {
  getAllpenerbit: async (req, res, next) => {
    try {
      const keyword = req.query.keyword || "";
      const page = Number(req.query.page) || 1;
      const limit = Number(req.query.limit) || 10;
      const offset = (page - 1) * limit;
      const sortby = req.query.sortby || "id_penerbit";
  ...
  getSearchpenerbit: async (req, res, next) => {
    try {
      const sort = req.query.sort || "ASC";
      const keyword = req.query.keyword || "";
      const result = await selectSearchpenerbit({ keyword, sort });
      // res.send(result)
  ...
  getDetailpenerbit: async (req, res, next) => {
  ...
  createpenerbit: async (req, res, next) => {
    ...
    const id_penerbit = Number(count.count) + 1;
  ...
  updatepenerbit: async (req, res, next) => {
    try {
      const PORT = process.env.PORT || 4000;
      const DB_HOST = process.env.DB_HOST || "localhost";
      const id = Number(req.params.id);
  ...
  deletepenerbit: async (req, res, next) => {
    try {
      const id = Number(req.params.id);
      const { rowCount } = await findId(id);
      if (!rowCount) {
